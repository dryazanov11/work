...
class TestValidator(BaseCase):
    ...
    def test_negative_validator(self):
        ...
        Assertions.assert_json_value_by_name(nomessage_error, 'message', "MessageOnError is required parameter\nParameter name: messageOnError",
        ...
        Assertions.assert_json_value_by_name(noexpression, 'message', "Expression is required parameter\nParameter name: expression",
                                             'Неожиданная ошибка при создании внутреннего валидатора без expression')
        ...
        unexist_id_update = MyRequests.post(f'/tm-core/api/Commands/UpdateValidator/{config.default_id}',headers={'Content-Type': 'application/json-patch+json','Authorization': f'{config.token_tm_core}'},
                                       data=self.create_validator)
        Assertions.assert_json_value_by_name(unexist_id_update, 'message', 'Validator with id = 3fa85f64-5717-4562-b3fc-2c963f66afa6 not found.',
        ...
        unexist_id_delete = MyRequests.delete(f'/tm-core/api/Commands/DeleteValidator/{config.default_id}',headers={'Content-Type': 'application/json-patch+json',
                                                     'Authorization': f'{config.token_tm_core}'})
        Assertions.assert_json_value_by_name(unexist_id_delete, 'message','Validator with id = 3fa85f64-5717-4562-b3fc-2c963f66afa6 not found.',
                                             'Ожидаемая ошибка не получена')
    ...
    def test_create_update_get_delete_validator(self):
        ...
        Assertions.assert_json_value_by_name(delete, 'success', True, 'Удаление валидатора прошло неуспешно')
...
class TestExternalValidator(BaseCase):
    ...
    def test_negative_externalvalidator(self):
        ...
        Assertions.assert_json_value_by_name(noname, 'message', 'validatorName is required parameter', 'Неожиданная ошибка при создании внешнего валидатора без name')
        ...
        Assertions.assert_json_value_by_name(nourl, 'message', "Url is required parameter\nParameter name: url",
                                             'Неожиданная ошибка при создании внешнего валидатора без url')

        #не передан MessageOnError
        ...
        Assertions.assert_json_value_by_name(nomessage, 'message', "MessageOnError is required parameter\nParameter name: messageOnError",
                                             'Неожиданная ошибка при создании внутреннего валидатора без messageOnError')
        ...
        #обновление с несуществующим id
        unexist_id_update = MyRequests.post(f'/tm-core/api/Commands/UpdateExternalValidator/{config.default_id}',headers={'Content-Type': 'application/json-patch+json','Authorization': f'{config.token_tm_core}'},
                                            data=self.create_exvalidator)
        Assertions.assert_json_value_by_name(unexist_id_update, 'message',
                                             'Validator with id = 3fa85f64-5717-4562-b3fc-2c963f66afa6 not found.',
                                             'Ожидаемая ошибка не получена')
...
class TestCallback(BaseCase):
    ...
    def test_negative_callback(self):
        ...
        Assertions.assert_json_value_by_name(nourl, 'message', 'Url is required parameter\nParameter name: url',
                                             'Неожиданная ошибка при создании callback без url')
